...
batch_dft_prdft := function(conf, t) 
...
#3D DFT/Batch DFT
3d_dft_batchdft := function(conf, t)
    local tt, _tt, _tt2, _conf, _opts, _HPCSupportedSizesCUDA, _thold,
    MAX_KERNEL, MAX_PRIME, MIN_SIZE, MAX_SIZE, size1, filter;
...
3d_dft_idft_nonconv := function(conf, t) 
    local tt, _tt, _tt2, _conf, _opts, _HPCSupportedSizesCUDA, _thold,
    MAX_KERNEL, MAX_PRIME, MIN_SIZE, MAX_SIZE, size1, filter;
...
tfcall_hockney_130 := function(conf, t) 
    local tt, _tt, _tt2, _conf, _opts, _HPCSupportedSizesCUDA, _thold,
    MAX_KERNEL, MAX_PRIME, MIN_SIZE, MAX_SIZE, size1, filter;
...
tfcall_hockney_general := function(conf, t) 
    local tt, _tt, _tt2, _conf, _opts, _HPCSupportedSizesCUDA, _thold,
    MAX_KERNEL, MAX_PRIME, MIN_SIZE, MAX_SIZE, size1, filter;
...
default_case := function(conf, t) 
    return [true, FFTXGlobals.getOpts(conf)]; 
end;

ParseOptsCUDA := function(conf, t)
    local list, _opts, bool, itr;
    list := [batch_dft_prdft, 3d_dft_batchdft, 3d_dft_idft_nonconv,
            tfcall_conv, tfcall_hockney_130, tfcall_hockney_general, 
            warpx, default_case];
    _opts := false;
    bool := false;
    itr := 1;
    if IsBound(conf.useCUDADevice) then 
        while bool = false do
            [bool, _opts] := ApplyFunc(list[itr], [conf, t]);
            itr := itr + 1;
        od;
    fi;
    ...
    if _opts <> false then
        Print("Found the opts in my parse opts\n");
        return _opts;
    else   
...
ParseOptsCUDA2 := function(conf, t)
    local tt, _tt, _tt2, _conf, _opts, _HPCSupportedSizesCUDA, _thold,
    MAX_KERNEL, MAX_PRIME, MIN_SIZE, MAX_SIZE, size1, filter;
